...
<div class="bg-white rounded-lg shadow max-w-2xl mx-auto">
    <div class="p-6 border-b">
        <h3 class="text-lg font-semibold">Form Edit Kategori</h3>
    </div>
    
    <form action="{{ route('admin.kategoris.update', $kategori) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <textarea name="deskripsi" rows="3"
                      class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-blue-500 @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
            @error('deskripsi')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
